...
def test_run_shake_help():
    """Test 'run shake --help'."""
    runner = CliRunner()
    result = runner.invoke(cli, ["run", "shake", "--help"])
    assert result.exit_code == 0
    assert "--filelist" in result.output
    assert "--rattles" in result.output
    assert "--prepare-only" in result.output
    assert "--calculator" in result.output


def test_run_shake_prepare_only_accepts_files_and_filelist(tmp_path):
    """Shake preparation writes deterministic RES inputs from args and filelist."""
    runner = CliRunner()
    with runner.isolated_filesystem(temp_dir=tmp_path):
        Path("Si-001.res").write_text(
            "TITL Si-001 0.000 125.000 -10.0000 0.00 0.00 2 (P1) n - 1\n"
            "CELL 1.0 5.000000 5.000000 5.000000 90.000000 90.000000 90.000000\n"
            "LATT -1\n"
            "SFAC Si\n"
            "Si 1 0.0000000000000 0.0000000000000 0.0000000000000 1.0\n"
        ...
        Path("C-001.res").write_text(
            "TITL C-001 0.000 64.000 -8.0000 0.00 0.00 2 (P1) n - 1\n"
            "CELL 1.0 4.000000 4.000000 4.000000 90.000000 90.000000 90.000000\n"
            "LATT -1\n"
            "SFAC C\n"
            "C 1 0.0000000000000 0.0000000000000 0.0000000000000 1.0\n"
            "C 1 0.2500000000000 0.2500000000000 0.2500000000000 1.0\n"
            "END\n"
        )
        Path("inputs.txt").write_text("# filelist entry\nC-001.res\n")
        ...
        shaken = sorted(Path("shake-work/inputs").glob("*.res"))
        assert [path.name for path in shaken] == [
            "C-001-shake01.res",
            "C-001-shake02.res",
            "Si-001-shake01.res",
            "Si-001-shake02.res",
        ]
        rows = Path("shake-work/shake_inputs.csv").read_text()
        assert "source_path,source_label,shake_label" in rows
        assert "Si-001,Si-001-shake01" in rows
        assert "C-001,C-001-shake02" in rows


def test_run_shake_relax_summary_uses_collected_result_status(tmp_path):
    """Collected shake relaxations are summarized from real outputs, not inputs."""

    class FakeRunner:
        def clean_failed(self, struct_name):
            raise AssertionError(f"unexpected clean_failed for {struct_name}")

    def fake_collect(struct_name, code, calculator_spec=None):
        Path(f"{struct_name}.res").write_text(
            f"TITL {struct_name} 0.000 60.000 -12.0000 0.00 0.00 2 (P1) n - 1\n"
            "CELL 1.0 4.000000 4.000000 4.000000 90.000000 90.000000 90.000000\n"
            "LATT -1\n"
            "SFAC C\n"
            "C 1 0.0000000000000 0.0000000000000 0.0000000000000 1.0\n"
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=FakeRunner()),
            patch.object(cmd_run, "_run_local_structure_one", return_value=0),
            patch.object(cmd_run, "_collect_result", side_effect=fake_collect),
        ):
            result = runner.invoke(
                cli,
                [
        ...
        assert result.exit_code == 0, result.output
        rows = list(csv.DictReader(Path("shake-work/shake_summary.csv").open()))
        assert rows[0]["status"] == "relaxed"
        assert rows[0]["relaxed_enthalpy_per_atom"] == "-6"
        assert rows[0]["delta_mev_per_atom"] == "-2000"


def test_run_shake_relax_summary_marks_skipped_without_fake_energy(tmp_path):
    """Skipped relaxations must not be reported as relaxed placeholder inputs."""
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=FakeRunner()),
            patch.object(cmd_run, "_walltime_remaining_ok", return_value=False),
            patch.object(cmd_run, "_collect_result") as collect_result,
        ):
            result = runner.invoke(
                cli,
                [
        ...
        assert rows[0]["relaxed_path"] == ""
        assert rows[0]["relaxed_enthalpy_per_atom"] == ""
        assert rows[0]["delta_mev_per_atom"] == ""


def test_run_shake_castep_uses_template_cell_and_param(tmp_path):
    """DFT-backed shake relaxation should use explicit templates via runners."""

    class FakeRunner:
        def __init__(self):
            self.calls = []

        def run(self, struct_name, cell_content, param_input):
            self.calls.append((struct_name, cell_content, param_input))
            Path(f"{struct_name}.castep").write_text("fake castep output\n")
            return 0
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        Path("template.param").write_text(
            "task : geometryoptimization\ncut_off_energy : 300 eV\n"
        )
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=fake_runner),
            patch.object(cmd_run, "_collect_result", side_effect=fake_collect),
        ):
            result = runner.invoke(
                cli,
        ...
        struct_name, cell_content, param_input = fake_runner.calls[0]
        assert struct_name == "C-001-shake01"
        assert "%BLOCK LATTICE_CART" in cell_content
        assert "kpoint_mp_grid 2 2 2" in cell_content
        assert "cut_off_energy" in str(param_input).lower()
        rows = list(csv.DictReader(Path("shake-work/shake_summary.csv").open()))
        assert rows[0]["status"] == "relaxed"


def test_run_shake_castep_uses_source_seed_prefix_inputs(tmp_path):
    """DFT shake should infer seed-prefixed inputs from the source RES label."""
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        Path("C.param").write_text("task : geometryoptimization\n")
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=fake_runner),
            patch.object(cmd_run, "_collect_result", side_effect=fake_collect),
        ):
        ...
        assert result.exit_code == 0, result.output
        assert len(fake_runner.calls) == 1
        _, cell_content, param_input = fake_runner.calls[0]
        assert "kpoint_mp_grid 2 2 2" in cell_content
        assert "geometryoptimization" in str(param_input).lower()


def test_run_shake_dft_requires_seed_param_or_template_param(tmp_path):
    """DFT-backed shake relaxation needs seed-prefixed or explicit parameters."""
    runner = CliRunner()
    with runner.isolated_filesystem(temp_dir=tmp_path):
        Path("C-001.res").write_text(
            "TITL C-001 0.000 64.000 -8.0000 0.00 0.00 2 (P1) n - 1\n"
    ...
    assert result.exit_code != 0
    assert "Param file not found" in result.output
    assert "C.param" in result.output


def test_run_shake_castep_preserves_res_spin_tags(tmp_path):
    """Magnetic RES inputs should keep per-site spins after shaking."""
    ...
    def fake_collect(struct_name, code, calculator_spec=None):
        Path(f"{struct_name}.res").write_text(
            ...
            "LATT -1\n"
            "SFAC Fe O\n"
            "Fe 1 0.0000000000000 0.0000000000000 0.0000000000000 1.0 4.000\n"
            "O 2 0.2500000000000 0.2500000000000 0.2500000000000 1.0 0.000\n"
            "END\n"
        )

    runner = CliRunner()
    with runner.isolated_filesystem(temp_dir=tmp_path):
        Path("FeO-001.res").write_text(
            "TITL FeO-001 0.000 64.000 -8.0000 0.00 0.00 2 (P1) n - 1\n"
            "CELL 1.0 4.000000 4.000000 4.000000 90.000000 90.000000 90.000000\n"
            "LATT -1\n"
            "SFAC Fe O\n"
            "Fe 1 0.0000000000000 0.0000000000000 0.0000000000000 1.0 4.000\n"
        ...
        Path("template.param").write_text("task : geometryoptimization\n")
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=fake_runner),
            patch.object(cmd_run, "_collect_result", side_effect=fake_collect),
        ):
        ...
        assert result.exit_code == 0, result.output
        assert "SPIN=4.000" in fake_runner.cell_content
        assert "SPIN=0.000" in fake_runner.cell_content


def test_run_shake_vasp_passes_template_kpoints(tmp_path):
    """VASP-backed shake should accept explicit template KPOINTS."""

    class FakeRunner:
        def __init__(self):
            self.kpoints_path = None
            self.last_metadata = {}

        def run(self, struct_name, cell_content, incar_text, kpoints_path=None):
            self.kpoints_path = kpoints_path
            Path(f"{struct_name}.vasp").mkdir()
            return 0
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        Path("INCAR").write_text("ENCUT = 300\n")
        Path("KPOINTS").write_text("Automatic mesh\n0\nGamma\n2 2 2\n0 0 0\n")
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=fake_runner),
            patch.object(cmd_run, "_collect_result", side_effect=fake_collect),
        ):
        ...
        assert result.exit_code == 0, result.output
        assert fake_runner.kpoints_path == Path("KPOINTS").resolve()
        assert fake_runner.kpoints_path.read_text() == (
            "Automatic mesh\n0\nGamma\n2 2 2\n0 0 0\n"
        )


def test_run_shake_vasp_uses_source_seed_prefix_inputs(tmp_path):
    """VASP-backed shake should infer <seed>.INCAR and <seed>.KPOINTS by default."""
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        Path("C.INCAR").write_text("ENCUT = 300\n")
        Path("C.KPOINTS").write_text("Automatic mesh\n0\nGamma\n2 2 2\n0 0 0\n")
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=fake_runner),
            patch.object(cmd_run, "_collect_result", side_effect=fake_collect),
        ):
        ...
        assert result.exit_code == 0, result.output
        assert fake_runner.incar_text == "ENCUT = 300\n"
        assert fake_runner.kpoints_path == Path("C.KPOINTS").resolve()
        assert fake_runner.kpoints_path.read_text() == (
            "Automatic mesh\n0\nGamma\n2 2 2\n0 0 0\n"
        )


def test_run_shake_vasp_finds_seed_inputs_beside_source_res(tmp_path):
    """Seed-prefixed DFT inputs should be resolved beside each source RES."""
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        search_dir.mkdir()
        (search_dir / "C-001.res").write_text(
            "TITL C-001 0.000 64.000 -8.0000 0.00 0.00 2 (P1) n - 1\n"
            "CELL 1.0 4.000000 4.000000 4.000000 90.000000 90.000000 90.000000\n"
            "LATT -1\n"
            "SFAC C\n"
        ...
        (search_dir / "C.INCAR").write_text("ENCUT = 300\n")
        (search_dir / "C.KPOINTS").write_text(
            "Automatic mesh\n0\nGamma\n2 2 2\n0 0 0\n"
        )
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=fake_runner),
        ...
        assert result.exit_code == 0, result.output
        assert fake_runner.incar_text == "ENCUT = 300\n"
        assert fake_runner.kpoints_path == (search_dir / "C.KPOINTS").resolve()
        assert fake_runner.kpoints_path.read_text() == (
            "Automatic mesh\n0\nGamma\n2 2 2\n0 0 0\n"
        )


def test_run_shake_vasp_keeps_per_source_seed_inputs_separate(tmp_path):
    """Same seed prefix from different source dirs should keep distinct inputs."""

    class FakeRunner:
        def __init__(self):
            self.incar_by_structure = {}
            self.kpoints_by_structure = {}
            self.last_metadata = {}

        def run(self, struct_name, cell_content, incar_text, kpoints_path=None):
            self.incar_by_structure[struct_name] = incar_text
            self.kpoints_by_structure[struct_name] = kpoints_path.read_text()
            Path(f"{struct_name}.vasp").mkdir()
            return 0
    ...
    runner = CliRunner()
    with runner.isolated_filesystem(temp_dir=tmp_path):
        for dirname, label, encut, kmesh in (
            ("search-a", "C-001", "ENCUT = 300\n", "1 1 1"),
            ("search-b", "C-002", "ENCUT = 500\n", "2 2 2"),
        ):
            search_dir = Path(dirname)
            search_dir.mkdir()
            (search_dir / f"{label}.res").write_text(
                f"TITL {label} 0.000 64.000 -8.0000 0.00 0.00 2 (P1) n - 1\n"
                "CELL 1.0 4.000000 4.000000 4.000000 90.000000 90.000000 90.000000\n"
                "LATT -1\n"
                "SFAC C\n"
                "C 1 0.0000000000000 0.0000000000000 0.0000000000000 1.0\n"
            ...
            (search_dir / "C.INCAR").write_text(encut)
            (search_dir / "C.KPOINTS").write_text(
                f"Automatic mesh\n0\nGamma\n{kmesh}\n0 0 0\n"
            )
        with (
        ...
        assert result.exit_code == 0, result.output
        assert fake_runner.incar_by_structure == {
            "C-001-shake01": "ENCUT = 300\n",
            "C-002-shake01": "ENCUT = 500\n",
        }
        assert "1 1 1" in fake_runner.kpoints_by_structure["C-001-shake01"]
        assert "2 2 2" in fake_runner.kpoints_by_structure["C-002-shake01"]


def test_run_shake_seed_prefix_prefers_source_filename(tmp_path):
    """The seed prefix should come from the source RES filename, not TITL."""
    ...
    runner = CliRunner()
    with runner.isolated_filesystem(temp_dir=tmp_path):
        Path("Renamed-001.res").write_text(
            "TITL Wrong-001 0.000 64.000 -8.0000 0.00 0.00 2 (P1) n - 1\n"
            "CELL 1.0 4.000000 4.000000 4.000000 90.000000 90.000000 90.000000\n"
            "LATT -1\n"
            "SFAC C\n"
            "C 1 0.0000000000000 0.0000000000000 0.0000000000000 1.0\n"
        ...
        Path("Renamed.cell").write_text(
            "%BLOCK LATTICE_CART\n4 0 0\n0 4 0\n0 0 4\n%ENDBLOCK LATTICE_CART\n"
        )
        Path("Renamed.param").write_text("task : geometryoptimization\n")
        with (
            patch.object(cmd_run, "_create_task_runner", return_value=FakeRunner()),
            patch.object(cmd_run, "_collect_result", side_effect=fake_collect),
        ):
...
def test_run_shake_keep_stale_archives_old_artifacts(tmp_path):
    """--keep-stale keeps old deterministic artifacts without reusing them."""

    class FakeRunner:
        def run(self, struct_name, cell_content, param_input):
            assert not Path(f"{struct_name}.castep").exists()
            Path(f"{struct_name}.castep").write_text("new castep output\n")
            return 0
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        Path("template.param").write_text("task : geometryoptimization\n")
        workdir = Path("shake-work/relax")
        workdir.mkdir(parents=True)
        (workdir / "C-001-shake01.castep").write_text("old castep output\n")
        (workdir / "C-001-shake01.vasp").mkdir()
        (workdir / "C-001-shake01.vasp" / "OUTCAR").write_text("old outcar\n")
        ...
        assert result.exit_code == 0, result.output
        stale_dir = Path("shake-work/stale/C-001-shake01/0001")
        assert (stale_dir / "C-001-shake01.castep").read_text() == "old castep output\n"
        assert (stale_dir / "C-001-shake01.vasp" / "OUTCAR").read_text() == "old outcar\n"
        assert Path("shake-work/relax/C-001-shake01.castep").read_text() == (
            "new castep output\n"
        )


def test_run_shake_default_removes_stale_artifacts(tmp_path):
    """Default shake reruns delete stale deterministic artifacts before running."""

    class FakeRunner:
        def run(self, struct_name, cell_content, param_input):
            assert not Path(f"{struct_name}.castep").exists()
            assert not Path(f"{struct_name}-out.cell").exists()
            assert not Path(f"{struct_name}.vasp").exists()
            assert not Path(f"{struct_name}.abacus").exists()
            Path(f"{struct_name}.castep").write_text("new castep output\n")
            return 0
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        workdir.mkdir(parents=True)
        (workdir / "C-001-shake01.castep").write_text("old castep output\n")
        (workdir / "C-001-shake01-out.cell").write_text("old cell\n")
        (workdir / "C-001-shake01.vasp").mkdir()
        (workdir / "C-001-shake01.abacus").mkdir()
        ...
        assert result.exit_code == 0, result.output
        assert not Path("shake-work/stale").exists()
        assert not Path("shake-work/relax/C-001-shake01.vasp").exists()
        assert not Path("shake-work/relax/C-001-shake01.abacus").exists()


def test_run_shake_keep_stale_uses_distinct_archive_generations(tmp_path):
    """Repeated --keep-stale reruns preserve older archive generations."""
    relax_dir = tmp_path / "relax"
    archive_dir = tmp_path / "stale"
    relax_dir.mkdir()
    (relax_dir / "C-001-shake01.castep").write_text("first stale\n")

    cmd_run._clear_shake_stale_artifacts(
        relax_dir,
        "C-001-shake01",
        archive_dir=archive_dir,
    )
    (relax_dir / "C-001-shake01.castep").write_text("second stale\n")
    cmd_run._clear_shake_stale_artifacts(
        relax_dir,
        "C-001-shake01",
        archive_dir=archive_dir,
    ...
    archived = sorted(archive_dir.glob("C-001-shake01/*/C-001-shake01.castep"))
    assert len(archived) == 2
    assert [path.read_text() for path in archived] == ["first stale\n", "second stale\n"]
